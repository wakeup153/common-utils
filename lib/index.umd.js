(function (global, factory) {
   ...
   * @returns array
   */
  const arrayToTreeByRef = (arr = [], parentId = 'pid', children = 'children') => {
   ...
   * @returns array
   */
  const arrayToTreeByRecursion = (arr = [], pid = 0, id = 'id', parentId = 'pid', children = 'children') => {
    if (!Array.isArray(arr) || arr.length === 0) {
      return [];
    }
    let result = [];
   ...
   * @returns 
   */
  const treeToArrayByDFS = (tree = [], children = 'children') => {
   ...
   * @returns 
   */
  const treeToArrayByBFS = (tree = [], children = 'children') => {
    if (!Array.isArray(tree) || tree.length === 0) {
      return [];
    }
   ...
   * @returns 
   */
  const treeToArrayByRecursion = (tree = [], children = 'children') => {
    if (!Array.isArray(tree) || tree.length === 0) {
      return [];
    }
    let result = [];
   ...
   * @returns array
   */
  const filterTreeByFunc = (tree, func = () => {}, children = 'children') => {
    if (!Array.isArray(tree) || tree.length === 0) {
      return [];
    }
   ...
   * @returns 
   */
  const getNodePath = (tree, id, onlyGetIdArray = true, idField = 'id', children = 'children') => {
    if (!Array.isArray(tree) || tree.length === 0) {
      return [];
    }
    const path = [];
   ...
   * @returns array
   */
  const fuzzyQueryTree = (arr, field = 'name', value, children = 'children') => {
    if (!Array.isArray(arr) || arr.length === 0) {
      return [];
    }
    let result = [];
   ...
   * @returns 
   */
  const getTreeAllLeaf = (tree, children = 'children') => {
    const result = [];
    const getLeaf = (tree = []) => {
      tree.forEach(item => {
        if (!item[children]) {
  ...
  var tree = /*#__PURE__*/Object.freeze({
    __proto__: null,
    arrayToTreeByRef: arrayToTreeByRef,
    arrayToTreeByRecursion: arrayToTreeByRecursion,
    treeToArrayByDFS: treeToArrayByDFS,
    treeToArrayByBFS: treeToArrayByBFS,
    treeToArrayByRecursion: treeToArrayByRecursion,
    filterTreeByFunc: filterTreeByFunc,
    getNodePath: getNodePath,
    fuzzyQueryTree: fuzzyQueryTree,
    getTreeAllLeaf: getTreeAllLeaf
  });
   ...
   * @returns array
   */
  const flattenByOrigin = (arr = [], level = Infinity) => {
    if (!Array.isArray(arr)) {
      return arr;
    }
  ...
  var array = /*#__PURE__*/Object.freeze({
    __proto__: null,
    flattenByRecursion: flattenByRecursion,
    flattenByReduce: flattenByReduce,
    flattenByOrigin: flattenByOrigin,
    runQueue: runQueue
  });
